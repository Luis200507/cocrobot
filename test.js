...
const siteUrl = "https://passaporte.coc.com.br/giul/api/oauth2/authorize?client_id=eduqo&response_type=code&access_token=d19ee526-a77b-443d-8395-4596b808c565";
...
async function navegarEResponderModulos(page) {
  ...
  finalReport.incorrectQuestions.forEach(element => {
...
async function responderExers(page) {
  for (let atividadeAtual = 1; atividadeAtual <= numeroDeAtividades; atividadeAtual++) {
    ...
    exerciseTitle = await page.$eval('.qm-content-toolbar-title', e => {
      return e.getAttribute('title')
    });
    console.log(`Respondendo atividade número ${atividadeAtual}: ${exerciseTitle}`);
